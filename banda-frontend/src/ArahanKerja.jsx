...
function ArahanKerja() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Work Order Issued!', { description: `Contractor assigned for report #${selectedAduan.id_aduan}.` });
      setAduans(aduans.filter(a => a.id_aduan !== selectedAduan.id_aduan));
      setSelectedAduan(null);
    } catch (error) {
      toast.error('Error', { description: error.response?.data?.message || 'Failed to issue work order.' });
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <main className="flex-1 overflow-auto p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6 overflow-y-auto pr-1">
            <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Clock className="w-4 h-4" /> Pending Assignment ({aduans.length})
            </h4>
              ...
              <div className="space-y-4">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
                            Reported: {new Date(aduan.tarikh_lapor).toLocaleDateString('ms-MY')}
                          </p>
                          {hasCluster ? (
                            ...
                            <div className="mx-4 mb-4 rounded-2xl border border-purple-100 bg-purple-50 overflow-hidden">
                              ...
                              <div className="px-4 py-3 bg-purple-100 flex items-center gap-2 border-b border-purple-200">
                                ...
                                <span className="text-xs font-black text-purple-700 uppercase tracking-wider">
                                  {aduan.anak_aduan_count} Related Reports within 20m Radius
                                </span>
                              </div>
                              {/* Child aduan list */}
                              ...
                              <div className="px-4 py-2.5 bg-purple-100/60 border-t border-purple-200">
                                <p className="text-[9px] text-purple-600 font-bold">
                                  ℹ️ One Work Order will resolve all {1 + aduan.anak_aduan_count} grouped reports in this cluster.
                                </p>
                              </div>
                            </div>
                          </motion.div>
                ...
                {aduans.length === 0 && (
                  <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 border-dashed">
                    <p className="text-slate-400 font-medium">No reports pending assignment at this time.</p>
                  </div>
                )}
              </div>
            )}
            ...
            <AnimatePresence mode="wait">
              {selectedAduan ? (
                ...
                >
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-5">
                      ...
                      <div className="grid grid-cols-2 gap-5">
                        <div>
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Expected Completion Date</label>
                          <input 
                        ...
                        <div>
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Estimated Cost (RM)</label>
                          <input 
                      ...
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Work Order Instructions</label>
                        <textarea 
                      ...
                      <div className={`p-5 rounded-2xl border-2 transition-all ${isOverBudget ? 'bg-rose-50 border-rose-200' : 'bg-slate-50 border-slate-200'}`}>
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <h6 className={`text-xs font-black uppercase tracking-widest ${isOverBudget ? 'text-rose-600' : 'text-slate-500'}`}>Department Budget Status</h6>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-sm mb-2 pb-2 border-b border-slate-200/60">
                          <span className="text-slate-500 font-medium">Deduct Job Cost:</span>
                        ...
                        {isOverBudget && (
                          <div className="mt-3 flex items-start gap-2 bg-rose-100 p-3 rounded-xl text-rose-700 text-xs font-bold">
                            ...
                            <p>Warning: Insufficient budget. Please ensure estimated cost does not exceed available balance.</p>
                          </div>
                        )}
                      </div>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-12 bg-slate-100/50 border-2 border-dashed border-slate-200 rounded-[40px]">
                  ...
                  <h4 className="text-lg font-black text-slate-400">Select a Report</h4>
                  <p className="text-xs text-slate-400 max-w-[200px]">Please select any pending report from the list on the left to assign a contractor.</p>
                </div>
              )}
            </AnimatePresence>
          </div>
