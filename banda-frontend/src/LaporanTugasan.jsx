...
function LaporanTugasan() {
  ...
  const fetchData = async (isInitial = true) => {
    try {
      ...
      // Fetch arahan kerja
      const res = await axios.get(`/api/pegawai/arahan-kerja?t=${Date.now()}`, {
  ...
  const hantarKomen = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `/api/pegawai/arahan-kerja/${selectedTask.id_arahan}/log`,
        { nota: komen },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const hantarPengesahan = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `/api/pegawai/arahan-kerja/${selectedTask.id_arahan}/sahkan`,
        sahkanData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
