  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      if (file.size > 20 * 1024 * 1024) {
        toast.error('Image Too Large', { description: 'Maximum size is 20MB.'});
        return;
      }
  ...
  const handleAdditionalFileChange = (e) => {
    ...
    const validFiles = files.filter(file => {
      ...
      if (isImage && file.size > 20 * 1024 * 1024) {
        toast.error('Image Too Large', { description: `${file.name} exceeds 20MB limit.` });
        return false;
      }
      ...
      <div className="relative z-10 flex justify-between">
        {[1, 2, 3].map((step) => (
          <div key={step} className="flex flex-col items-center">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold border-4 transition-colors duration-300 ${currentStep > step ? 'bg-blue-800 border-blue-800 text-white' :
                      ...
                      <div className="flex flex-col gap-6 bg-slate-50 p-6 sm:p-8 rounded-3xl border border-slate-200">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <button
                            ...
                            className={`mt-2 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all shadow-sm border ${isListening
