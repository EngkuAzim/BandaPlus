        ...
        <motion.div variants={itemVariants} className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 md:p-8 overflow-hidden">
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              tugasanList.map(t => (
                <div key={t.id_arahan} className="border border-slate-100 bg-slate-50 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-blue-200 transition-colors">
                  <div>
                  ...
                  <Link to="/pembaikan" className="bg-slate-900 hover:bg-blue-800 text-center text-white font-bold text-sm px-5 py-2.5 rounded-xl transition-all w-full sm:w-auto">
                    Update / Upload Proof
                  </Link>
                </div>
              ))
