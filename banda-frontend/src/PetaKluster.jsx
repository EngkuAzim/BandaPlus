...
function PetaKluster() {
  ...
  const displayPriority = (p) => ({ 'SANGAT TINGGI': 'Very High', 'TINGGI': 'High', 'SEDERHANA': 'Medium', 'RENDAH': 'Low' })[p] || p || 'Medium';
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-8 py-5 bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              <MapIcon className="w-6 h-6 text-violet-600" /> AI Cluster Mapping (3D)
            </h2>
            <p className="text-sm text-slate-500 font-medium mt-1">Real-time spatial monitoring of facility damage (500m radius)</p>
          </div>
          ...
          <div className="flex items-center gap-3 mt-4 sm:mt-0 bg-violet-50 border border-violet-200 px-4 py-2 rounded-xl">
            ...
            <span className="text-sm font-bold text-violet-900">Mapbox 3D Engine Active</span>
          </div>
        </header>
            ...
            <>
                    ...
                    >
                      <div className="p-1">
                        ...
                        <div className="px-2 pb-2 pt-1">
                          ...
                          >
                            {displayPriority(popupInfo.label_prioriti)}
                          </span>
                          <h4 className="font-bold text-slate-800 mt-2 leading-tight">{popupInfo.jenis}</h4>
                          <p className="text-xs text-slate-500 mt-1 line-clamp-2">AI Score: <span className="font-bold text-slate-900">{popupInfo.weight}%</span></p>
                          {popupInfo.anak_aduan_count > 0 && (
                            <div className="mt-2 flex items-center gap-1 bg-purple-100 text-purple-700 px-2 py-1 rounded text-[10px] font-bold">
                              ...
                              {popupInfo.anak_aduan_count} Clustered Reports
                            </div>
                          )}
              ...
              >
                <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                  <h3 className="font-black text-slate-900 flex items-center gap-2 mb-4"><Layers className="w-5 h-5 text-violet-600" /> 3D Spatial Analytics</h3>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-white border border-slate-200 p-3 rounded-xl text-center shadow-sm">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Detected Clusters</p>
                    </div>
                  </div>
                </div>

                <div className="p-6 flex-1">
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Cluster Alerts (500m Radius)</h4>
                  ...
                  <div className="space-y-4">
                          ...
                          <div className="flex items-start gap-3 relative z-10">
                            ...
                            <div>
                              <h5 className={`font-bold text-sm ${isHighRisk ? 'text-rose-900' : 'text-amber-900'}`}>
                                {isHighRisk ? 'High-Risk Cluster' : 'Overlapping Reports Detected'}
                              </h5>
                              <p className={`text-xs font-medium mt-1 ${isHighRisk ? 'text-rose-700/80' : 'text-amber-700/80'}`}>
                                {totalAduan} reports of {kluster.jenis} detected near {kluster.alamat_lokasi}.
                              </p>
                              <div className="mt-3 flex items-center gap-2">
                                <span className={`text-[10px] px-2 py-1 rounded font-black uppercase ${isHighRisk ? 'bg-rose-200 text-rose-800' : 'bg-amber-200 text-amber-800'}`}>
                                  Priority: {displayPriority(kluster.label_prioriti)}
                                </span>
                              </div>
                            </div>
                          </div>
                    ...
                    {aduans.filter(a => a.anak_aduan_count > 0).length === 0 && (
                      <div className="text-center p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        <p className="text-xs font-bold text-slate-400">No clusters detected at this time.</p>
                      </div>
                    )}
                  </div>
                  
                  <div className="mt-8 p-4 bg-slate-900 text-white rounded-2xl text-xs leading-relaxed opacity-80">
                    <span className="font-bold text-violet-400 block mb-1">Algorithm Note:</span>
                    BANDA+'s spatial clustering algorithm groups complaint points within a 500-meter radius into a single Parent Node using MySQL spatial index. 3D building extrusions are dynamically rendered via Mapbox.
                  </div>
                </div>
              </motion.div>
            </>
